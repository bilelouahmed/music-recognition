        ...
        for j in range(1, fan_value):
            if (i + j) < len(peaks):
                ...
                if 0 <= time_delta <= 200:
                    ...
                    hash_string = f"{freq1:.2f}|{freq2:.2f}|{time_delta}"

                    hash_pairs.append((hash_string, time1))

    return hash_pairs


def store_fingerprints(
    db: FingerprintsDatabase, song_details: dict, fingerprint: list
) -> int:
    """
    Store the fingerprint in the database.
    """
    song_id = db.insert_song(song_details)
    db.insert_fingerprint(song_id, fingerprint)
